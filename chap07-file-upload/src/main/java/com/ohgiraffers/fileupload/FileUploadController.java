...
public class FileUploadController {
    ...
    public String singleFileUpload(@RequestParam String singleFileDescription, @RequestParam MultipartFile singleFile, Model model){
        ...
        /* 서버로 전달된 file을 서버에서 설정하는 경로에 저장 */
        ...
        try {
            ...
            model.addAttribute("message", "파일업로드 완료");
        } catch (IOException e) {
    ...
    public String multiFileUpload(@RequestParam String multiFileDescription, @RequestParam List<MultipartFile> multiFile, Model model){
        ...
        try {
            ...
            /* 서버의 정해진 경로에 파일저장이 완료되면 List<FileDTO> 타입의 객체에 저장된 정보를 DB에 insert 한다. */
            model.addAttribute("message", "파일업로드 완료");

        } catch (IOException e) {
            /* 파일 저장 중간에 실패 시, 이전에 저장된 파일 삭제 */
            for(FileDTO file : files) new File(filePath + "/" + file.getSavedName()).delete();
            model.addAttribute("message", "파일업로드 실패");
        }
